...
chrome.spawn = function (options) {
	return new Promise((resolve, reject) => {
		...
		let chromeFlags = this.options.chromeFlags || [
			...
			'--remote-debugging-address=' + this.options.browserDebuggingHost,
			'--hide-scrollbars',
		];
        ...
        /*
          ROD - can't use {shell: true} on Mac, so we make it configurable
		  this.chromeChild = spawn(location, chromeFlags, {shell: true});
        */
		// `detached` will group all the processes so we can kill them easily
        let spawnFlags = this.options.spawnFlags || {shell: true, detached: true};

		this.chromeChild = spawn(location, chromeFlags, spawnFlags);
...
chrome.kill = function () {
	if (this.chromeChild) {
		...
		// negative PID means the whole group
		process.kill(-this.chromeChild.pid, signal);
	}
};
	...
	return new Promise((resolve, reject) => {
		...
		let connect = () => {
			CDP.Version({ host: this.options.browserDebuggingHost, port: this.options.browserDebuggingPort }).then((info) => {
				...
				util.log('Error while running CDP.Version (will retry)');
